...
import { computeDrawdown, findLastCorrectionPeak } from './drawdown';
...
function point(monthIndex: number, date: string, level: number): PortfolioIndexPoint {
  return { monthIndex, date, level, isSynthetic: false, priceUsd: null };
}
...
describe('computeDrawdown', () => {
  ...
  it('전역 최대낙폭과 그 고점·저점을 찾는다', () => {
    ...
    expect(result.peak.date).toBe('2000-02');
    expect(result.trough.date).toBe('2000-03');
    expect(result.recovery?.date).toBe('2000-05');
    expect(result.recoveryMonths).toBe(3);
  });

  it('시뮬레이션 종료까지 고점을 회복하지 못하면 recovery가 null이다', () => {
    const series = [point(0, '2000-01', 1.0), point(1, '2000-02', 2.0), point(2, '2000-03', 0.3)];
    const result = computeDrawdown(series);
  ...
  it('한 번도 하락하지 않으면 낙폭 0이다', () => {
    const series = [point(0, '2000-01', 1.0), point(1, '2000-02', 1.2), point(2, '2000-03', 1.5)];
    const result = computeDrawdown(series);
