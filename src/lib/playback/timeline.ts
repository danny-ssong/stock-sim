 ...
 *
 * 월 표기는 그 달의 1일로 본다 — 자산 시리즈(월별)가 가격 시리즈(일별)보다 그 달
 * 안에서 며칠 먼저 나타나지만, 재생 한 프레임이 며칠에 해당하므로 눈에 띄지 않는다.
 ...
 *
 * 진행도를 배열 인덱스가 아니라 **날짜**로 옮기는 것이 요점이다 — 인라인 화면은
 * 일별 가격 차트와 월별 자산 차트가 함께 움직이는데, 인덱스로 진행하면 두 차트가
 * 서로 다른 시점을 가리킨다.
 *
