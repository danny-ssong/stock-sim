 ...
 *
 * 두 차트는 같은 일별 해상도(둘 다 'YYYY-MM-DD')로 같은 기간을 덮지만, 렌더 전에
 * 각자 독립적으로 극값 보존 다운샘플링을 거치므로(lib/chart/downsample.ts) 실제로
 * 남는 점 집합·개수는 다를 수 있다. 그래서 틱을 "값이 있는 첫 항목"으로 잡으면
 * 두 축이 같은 시점에 같은 라벨을 찍는다. 해상도가 아니라 라벨 규칙을 공유하는 게 요점이다.
 *
 * 날짜의 앞 7글자('YYYY-MM')·앞 4글자('YYYY')만 쓰므로 'YYYY-MM' 표기를 넘겨도 된다.
 */
 ...
 * 위치를 날짜가 아니라 **배열 인덱스**로 잡으므로, 같은 틱 배열을 넘겨도 두 차트가
 * 독립적으로 다운샘플링을 거쳐 포인트 수가 달라지면(예: 가격 800개 vs 자산 650개)
 * minTickGap 충돌 판정이 다르게 나고, 결국 서로 다른 부분집합이 남는다 — 위는
 * 3·7·10월, 아래는 5·9월 식으로 갈렸다.
 *
